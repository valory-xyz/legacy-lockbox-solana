...
async function main() {
  ...
  const amount_in = new Decimal("1" /* sol */);
  const tradeAmount = DecimalUtil.toBN(amount_in, sol.decimals);
  ...
  const amountIn = DecimalUtil.fromBN(quote.estimatedAmountIn, sol.decimals);
  const amountOut = DecimalUtil.fromBN(quote.estimatedAmountOut, olas.decimals);
  console.log("estimatedAmountIn:", amountIn.toString(), "sol");
  console.log("estimatedAmountOut:", amountOut.toString(), "olas");
  console.log("otherAmountThreshold:", DecimalUtil.fromBN(quote.otherAmountThreshold, olas.decimals).toString(), "olas");
  const sol2olas = amountIn.toNumber() / amountOut.toNumber();
  const olas2sol = amountOut.toNumber() / amountIn.toNumber();
