...
import {
  createMint, mintTo, transfer, getOrCreateAssociatedTokenAccount, ASSOCIATED_TOKEN_PROGRAM_ID,
...
async function main() {
  ...
  const positionMint2 = positionMintKeypair.publicKey;
  console.log("positionMint2:", positionMint2.toBase58());
  const positionPda = PDAUtil.getPosition(orca, positionMint2);
  console.log("positionPda", positionPda);
  const position2 = positionPda.publicKey;
  console.log("position2", position2.toBase58());
  ...
  //const positionTokenAccount2 = await deriveATA(wallet.publicKey, positionMint);
...
//    try {
//      const signature = await program_whirlpool.methods.openPosition(
//        positionPda.bump,
//        lower_tick_index,
//        upper_tick_index)
//        .accounts(
